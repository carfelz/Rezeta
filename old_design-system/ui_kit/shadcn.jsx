...
const buttonVariants = {
  variant: {
    default:
      'bg-[color:var(--primary)] text-[color:var(--primary-foreground)] hover:bg-[#1B3A41] active:bg-[#0E2429]',
    secondary:
      'bg-[color:var(--card)] text-[color:var(--secondary-foreground)] border border-[color:var(--input)] hover:bg-[color:var(--secondary)]',
    ghost: 'text-[color:var(--foreground)] hover:bg-[color:var(--accent)]',
    destructive:
      'bg-[color:var(--destructive)] text-[color:var(--destructive-foreground)] hover:bg-[#6E2018]',
    outline:
...
const Button = React.forwardRef(
  ({ variant = 'default', size = 'default', className = '', children, ...p }, ref) => (
    <button
      ref={ref}
      className={cn(
...
const Label = ({ className = '', children, required, ...p }) => (
  <label
    className={cn(
      'block font-medium text-[12.5px] text-[color:var(--color-n-700)] mb-[6px]',
      className,
    )}
    {...p}
  >
...
const CardHeader = ({ className = '', children }) => (
  <div className={cn('flex flex-col gap-1 mb-3', className)}>{children}</div>
)
const CardTitle = ({ className = '', children }) => (
  <h3
    className={cn(
      'font-sans font-semibold text-[14px] text-[color:var(--color-n-800)] m-0',
      className,
    )}
  >
    {children}
...
const CardDescription = ({ className = '', children }) => (
  <p className={cn('text-[12.5px] text-[color:var(--color-n-500)] m-0 leading-[1.45]', className)}>
    {children}
  </p>
)
const CardContent = ({ className = '', children }) => (
  <div className={cn('text-[13px] text-[color:var(--color-n-700)]', className)}>{children}</div>
)
const CardFooter = ({ className = '', children }) => (
  <div
    className={cn(
      'flex items-center pt-3 mt-3 border-t border-[color:var(--color-n-100)]',
      className,
    )}
  >
    {children}
...
const badgeVariants = {
  draft:
    'bg-[color:var(--color-n-50)] border-[color:var(--color-n-200)] text-[color:var(--color-n-600)]',
  signed:
    'bg-[color:var(--color-p-50)] border-[color:var(--color-p-100)] text-[color:var(--color-p-700)]',
  active:
  ...
  review:
    'bg-[color:var(--color-warning-bg)] border-[color:var(--color-warning-border)] text-[color:var(--color-warning-text)]',
  overdue:
    'bg-[color:var(--color-danger-bg)] border-[color:var(--color-danger-border)] text-[color:var(--color-danger-text)]',
  archived:
    'bg-[color:var(--color-n-50)] border-[color:var(--color-n-200)] text-[color:var(--color-n-500)]',
...
const Switch = ({ checked, onCheckedChange, disabled, className = '' }) => (
  ...
  >
    <span
      className={cn(
        'inline-block h-[14px] w-[14px] rounded-full bg-white transition-transform duration-100',
...
const Checkbox = ({ checked, indeterminate, onCheckedChange, disabled, className = '' }) => (
  <button
    ...
    className={cn(
      ...
      checked || indeterminate
        ...
        : 'bg-white border-[color:var(--color-n-400)]',
  ...
  >
    {checked && !indeterminate && (
      <i className="ph ph-check" style={{ fontSize: 11, fontWeight: 'bold', lineHeight: 1 }} />
    )}
...
const Select = ({
  ...
  return (
    <div ref={ref} className={cn('relative', className)}>
      ...
      >
        <span className={current ? '' : 'text-[color:var(--color-n-400)]'}>
          {current?.label ?? placeholder}
        </span>
        <i
          className="ph ph-caret-down absolute right-3 top-1/2 -translate-y-1/2 text-[color:var(--color-n-500)]"
          style={{ fontSize: 14 }}
        />
      </button>
      {open && (
        <div className="absolute z-50 mt-1 w-full rounded-[5px] border border-[color:var(--border)] bg-[color:var(--popover)] shadow-[0_8px_24px_-8px_rgba(14,14,13,0.12),0_2px_6px_rgba(14,14,13,0.06)] py-1">
          {options.map((o) => (
            ...
            >
              <span>{o.label}</span>
              {o.value === value && (
                <i className="ph ph-check text-[color:var(--primary)]" style={{ fontSize: 13 }} />
              )}
            </button>
          ))}
        </div>
...
const Tabs = ({ value, onValueChange, defaultValue, children, className = '' }) => {
  ...
  return (
    <TabsContext.Provider value={{ value: v, set }}>
...
const TabsTrigger = ({ value, children, count }) => {
  ...
  return (
    <button
      ...
      className={cn(
        'relative -mb-px inline-flex items-center gap-2 px-4 py-3 text-[13px] font-medium transition-colors',
        active
          ? 'text-[color:var(--color-n-900)] border-b-2 border-[color:var(--primary)]'
          : 'text-[color:var(--color-n-500)] hover:text-[color:var(--color-n-800)] border-b-2 border-transparent',
      )}
    >
      {children}
      {count != null && (
        <span className="font-mono text-[11px] text-[color:var(--color-n-400)]">{count}</span>
      )}
    </button>
  )
}
...
const Dialog = ({ open, onOpenChange, children }) => {
  ...
  return ReactDOM.createPortal(
    ...
    >
      ...
      <div onClick={(e) => e.stopPropagation()} className="relative animate-[rise_150ms_ease]">
        {children}
      </div>
    </div>,
    document.body,
...
const DialogContent = ({ children, className = '' }) => (
  <div
    className={cn(
      'w-[440px] max-w-[calc(100vw-32px)] rounded-[5px] bg-[color:var(--card)] shadow-[0_1px_0_rgba(14,14,13,0.04),0_8px_24px_-8px_rgba(14,14,13,0.12),0_2px_6px_rgba(14,14,13,0.06)] overflow-hidden',
      className,
    )}
  >
    {children}
...
const DialogHeader = ({ children, className = '' }) => (
  <div className={cn('px-6 pt-5 pb-3.5 border-b border-[color:var(--color-n-100)]', className)}>
    {children}
  </div>
)
const DialogTitle = ({ children, className = '' }) => (
  <h2
    className={cn(
      'font-serif font-medium text-[19px] text-[color:var(--color-n-900)] m-0 tracking-[-0.005em]',
      className,
    )}
  >
    {children}
...
const DialogDescription = ({ children, className = '' }) => (
  <p className={cn('text-[13px] text-[color:var(--color-n-600)] m-0 mt-1', className)}>
    {children}
  </p>
)
...
const DialogFooter = ({ children, className = '' }) => (
  <div
    className={cn(
      'flex justify-end gap-3 px-5 py-3.5 bg-[color:var(--color-n-25)] border-t border-[color:var(--color-n-100)]',
      className,
    )}
  >
    {children}
...
const Table = ({ className = '', children }) => (
  <div className="rounded-[5px] border border-[color:var(--border)] bg-[color:var(--card)] overflow-hidden">
    <table className={cn('w-full border-collapse', className)}>{children}</table>
  </div>
)
const TableHeader = ({ children }) => (
  <thead className="bg-[color:var(--color-n-50)]">{children}</thead>
)
const TableBody = ({ children }) => <tbody>{children}</tbody>
const TableRow = ({ className = '', selected, ...p }) => (
  <tr
    className={cn(
      'border-b border-[color:var(--color-n-100)] last:border-b-0 hover:bg-[color:var(--color-n-25)] transition-colors',
      selected && 'bg-[color:var(--color-n-25)]',
      className,
    )}
    {...p}
  />
...
const TableHead = ({ className = '', children }) => (
  <th
    className={cn(
      'text-left font-sans font-semibold text-[11.5px] uppercase tracking-[0.06em] text-[color:var(--color-n-600)] px-4 py-3',
      className,
    )}
  >
    {children}
...
const TableCell = ({ className = '', children, mono, name }) => (
  <td
    className={cn(
      'px-4 py-3 text-[13px] text-[color:var(--color-n-700)]',
      name && 'font-semibold text-[color:var(--color-n-800)]',
      mono && 'font-mono text-[12px] text-[color:var(--color-n-600)]',
      className,
    )}
  >
    {children}
  ...
  return (
    <span ref={ref} className="relative inline-flex">
      ...
      {open && (
        <div
          className={cn(
            'absolute top-full mt-1 z-50 min-w-[200px] rounded-[5px] border border-[color:var(--border)] bg-[color:var(--popover)] shadow-[0_8px_24px_-8px_rgba(14,14,13,0.12),0_2px_6px_rgba(14,14,13,0.06)] py-1',
        ...
        >
          {items.map((it, i) =>
            it.separator ? (
              ...
              <div
                key={i}
                className="px-3 py-1 font-mono text-[10px] uppercase tracking-[0.1em] text-[color:var(--color-n-400)]"
              >
                {it.label}
              </div>
            ) : (
              <button
                ...
                className={cn(
                  ...
                  it.danger
                    ? 'text-[color:var(--destructive)] hover:bg-[color:var(--color-danger-bg)]'
              ...
              >
                {it.icon && (
                  <i
                    ...
                    style={{
                      fontSize: 14,
                      color: it.danger ? 'currentColor' : 'var(--color-n-500)',
                    }}
                  />
                )}
                <span className="flex-1">{it.children}</span>
                {it.shortcut && (
                  <span className="font-mono text-[10.5px] text-[color:var(--color-n-400)]">
                    {it.shortcut}
                  </span>
                )}
              </button>
  ...
  return ReactDOM.createPortal(
    <div className="fixed bottom-6 right-6 z-[600] flex flex-col gap-2 pointer-events-none">
      {items.map((t) => (
        <div
          key={t.id}
          className="pointer-events-auto w-[380px] rounded-[5px] border border-[color:var(--border)] bg-[color:var(--card)] shadow-[0_8px_24px_-8px_rgba(14,14,13,0.12),0_2px_6px_rgba(14,14,13,0.06)] p-4 flex gap-3 animate-[toastIn_180ms_ease]"
        >
          <i
            className={`ph ${iconFor[t.variant] || 'ph-info'} flex-shrink-0`}
            style={{ fontSize: 18, color: colorFor[t.variant] }}
          />
          <div className="flex-1 min-w-0">
            <div className="font-sans font-semibold text-[13px] text-[color:var(--color-n-800)]">
              {t.title}
            </div>
            {t.description && (
              <div className="text-[12.5px] text-[color:var(--color-n-500)] leading-[1.45] mt-0.5">
                {t.description}
              </div>
            )}
          </div>
          <button
            ...
            className="text-[color:var(--color-n-400)] hover:text-[color:var(--color-n-700)]"
          >
...
const Calendar = ({ value, onChange }) => {
  ...
  for (let w = 0; w < 6; w++) {
    const row = []
    for (let d = 0; d < 7; d++) {
      row.push(cur > 0 && cur <= days ? cur : null)
      cur++
    }
    weeks.push(row)
  }
    ...
    <div className="rounded-[5px] border border-[color:var(--border)] bg-[color:var(--card)] p-4 inline-block">
      <div className="flex items-center justify-between mb-3">
        <button
          ...
          className="w-7 h-7 rounded-[3px] hover:bg-[color:var(--accent)] flex items-center justify-center"
        >
      ...
      <div className="grid grid-cols-7 gap-0.5">
        {wd.map((d, i) => (
          <div
            key={i}
            className="font-mono text-[10px] uppercase tracking-[0.1em] text-[color:var(--color-n-400)] text-center py-1"
          >
            {d}
          </div>
        ))}
        {weeks.flat().map((d, i) => (
          <button
            ...
            className={cn(
              'h-8 w-8 rounded-[3px] text-[12px] font-sans transition-colors',
              !d && 'invisible',
...
const Command = ({ open, onOpenChange, items = [] }) => {
  ...
  const filtered = items.filter(
    (it) => !it.heading && (!q || it.label.toLowerCase().includes(q.toLowerCase())),
  )
  const grouped = []
  let lastH = null
  items.forEach((it) => {
  ...
  return ReactDOM.createPortal(
    <div
      className="fixed inset-0 z-[700] flex items-start justify-center pt-[12vh]"
      onClick={() => onOpenChange?.(false)}
    >
      <div className="absolute inset-0 bg-[rgba(14,14,13,0.35)]" />
      <div
        onClick={(e) => e.stopPropagation()}
        className="relative w-[560px] max-w-[calc(100vw-32px)] rounded-[8px] bg-[color:var(--card)] border border-[color:var(--border)] shadow-[0_8px_24px_-8px_rgba(14,14,13,0.12),0_2px_6px_rgba(14,14,13,0.06)] overflow-hidden"
      >
        <div className="flex items-center gap-3 px-4 border-b border-[color:var(--color-n-100)]">
          <i
            className="ph ph-magnifying-glass text-[color:var(--color-n-400)]"
            style={{ fontSize: 16 }}
          />
          <input
            autoFocus
            ...
            onKeyDown={(e) => {
              if (e.key === 'ArrowDown') {
                e.preventDefault()
                setIdx((i) => Math.min(i + 1, grouped.length - 1))
              }
              if (e.key === 'ArrowUp') {
                e.preventDefault()
          ...
          />
          <kbd className="font-mono text-[10.5px] text-[color:var(--color-n-500)] border border-[color:var(--color-n-200)] bg-[color:var(--color-n-25)] px-1.5 py-0.5 rounded-[3px]">
            esc
          </kbd>
        </div>
        <div className="max-h-[400px] overflow-y-auto py-1">
          {grouped.length === 0 && (
            <div className="py-10 text-center text-[13px] text-[color:var(--color-n-500)]">
              Sin resultados.
            </div>
          )}
          {grouped.map((it, i) => {
            ...
            return (
              <Fragment key={i}>
                {showHeading && it.heading && (
                  <div className="px-4 pt-2 pb-1 font-mono text-[10px] uppercase tracking-[0.1em] text-[color:var(--color-n-400)]">
                    {it.heading}
                  </div>
                )}
                <button
                  ...
                  className={cn(
                    'w-full text-left px-4 py-2 flex items-center gap-3 text-[13px]',
                    idx === i
                      ? 'bg-[color:var(--accent)] text-[color:var(--color-n-900)]'
                      : 'text-[color:var(--color-n-700)]',
                  )}
                >
                  {it.icon && (
                    <i
                      className={`ph ${it.icon} text-[color:var(--color-n-500)]`}
                      style={{ fontSize: 15 }}
                    />
                  )}
...
const Separator = ({ className = '' }) => (
  <div className={cn('h-px bg-[color:var(--border)]', className)} />
)
