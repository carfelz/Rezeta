...
- **Font size — always a token, never raw pixels.** The base UI text size is **`text-sm` (13px)** and the small size is **`text-xs` (12px)**; use these for body/UI text. The full scale is `text-2xs` (10) · `text-overline` (11) · `text-xs` (12) · `text-sm` (13) · `text-base` (14) · `text-body-lg` (16) · `text-h3` (18) · `text-h2` (28) · `text-h1` (40) · `text-display` (56). Round to the nearest token instead of inventing a size.
- **No arbitrary `prop-[value]` classes — ever.** Every size, spacing, color, radius, shadow, letter-spacing, line-height, z-index, etc. must map to a token in `apps/web/tailwind.config.ts`. An ESLint `no-restricted-syntax` guardrail **fails CI** on any design-value arbitrary class (`w-[440px]`, `py-[7px]`, `tracking-[0.06em]`, `bg-[#…]`, …). If the value you need has no token, **add it to the config** (and register custom `w-`/`h-`/`max-w-`/`tracking-`/… names in `cn`'s tailwind-merge per the coupling note below) rather than reaching for `-[…]`. **Exempt** (allowed): runtime CSS-var bindings (`w-[--cell-size]`, `[var(--radix-…)]`) and arbitrary *variant selectors* (`data-[state=open]:…`, `group-…`, `[&>span]:…`). The full token set and migration record: `docs/qa/2026-07-13-arbitrary-value-migration-scope.md`.
- **Adding a new custom font-size token requires two edits, not one.** Define it in `apps/web/tailwind.config.ts` (`theme.extend.fontSize`) **and** register the name in the `font-size` class group of `extendTailwindMerge` in `apps/web/src/lib/utils.ts`. `cn()` runs tailwind-merge, which only knows the stock size names; an unregistered custom token (e.g. `text-overline`) is misclassified as a text-*color* and gets stripped whenever a `cn()`-composed element also sets a color class — the element then silently inherits the body's 16px. The same coupling applies to any custom token whose name collides with a standard utility prefix (`text-`, `font-`, `h-`, `w-`).
- **`text-overline`/`text-caption`/`text-h1`/etc. are composite type styles, not just sizes** — via `@layer components` in `index.css` they also set font-family, weight, color, and (for `overline`) `uppercase`. Use them when you want that full style; for plain text that only needs a size, use the numeric tokens (`text-2xs`/`xs`/`sm`/`base`) so you don't inherit unwanted casing/family (this is why `Badge` uses `text-xs`, not `text-overline`).
