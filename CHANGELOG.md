...
- `apps/web/src/pages/settings/Users.tsx`: the create-user role `<select>` now filters its options to `canManageRole(actorRole, option)` via a new `manageableRoles()` helper, using the actor's role from `useAuth()`. Each roster row gained a rank-gated role-change `NativeSelect` (`RoleChangeControl`, wired to the existing `useChangeUserRole` hook) shown only when the actor outranks the row, with its own options filtered the same way; errors surface through the page's `Callout` pattern and the roster query invalidates on success (handled by the existing hook's `onSuccess`). Added a "Reenviar invitación" button (new `useResendInvite` hook in `apps/web/src/hooks/users/use-users.ts`, `POST /v1/users/:id/resend-invite`) shown only on rank-manageable rows with `status === 'invited'`. New strings in `apps/web/src/pages/settings/strings.ts`: `roleChangeLabel`, `roleChangeError`, `resendButton`, `resendingButton`, `resendError`.
- `apps/web/src/pages/settings/Users.tsx`: the activate/deactivate control is now hidden (not just disabled) on rows the actor cannot manage, matching the roster's existing conditional-render pattern.
- `apps/api/src/modules/users/users.service.ts`: `createUser` now survives partial failure across its three side effects (Firebase identity create → DB row → password-link email). A DB write failure after the Firebase identity was created triggers a best-effort compensating `authProvider.deleteUser(externalUid)` (via the already-existing `IAuthProvider.deleteUser`); the original error still propagates even if the cleanup itself throws (logged as a warning). A DB-side unique-constraint race (`P2002`) is mapped to `ConflictException({ code: ErrorCode.USER_ALREADY_EXISTS })` after cleanup. A failure in the password-reset-link/email step after the row was created is now non-fatal — logged as a warning, the created user is still returned (recovery path: resend).
- `apps/api/src/modules/users/users.service.ts`: new `resendInvite(tenantId, actorRole, actorUserId, targetUserId)` regenerates the set-password link and resends it, enforcing `canManageRole` + tenant scoping via the existing `requireUser`/`assertCanManage` helpers, and audits `user_invited` with `metadata: { role, resend: true }`.
- `apps/api/src/modules/users/users-management.controller.ts`: new `POST /v1/users/:id/resend-invite` (`@RequirePermission('users', 'manage')`, `@HttpCode(HttpStatus.OK)`), following the sibling endpoints' Swagger annotation conventions.
- `apps/api/src/lib/auth/firebase-auth.provider.ts`: `createUser` now catches the Firebase Admin SDK's `auth/email-already-exists` error code and throws `ConflictException({ code: ErrorCode.USER_ALREADY_EXISTS })` instead of a generic 500.
- Tests: `apps/api/src/lib/auth/__tests__/firebase-auth.provider.spec.ts` (duplicate-email mapping), `apps/api/src/modules/users/__tests__/users.service.spec.ts` (new `describe` blocks for invite-flow resilience — duplicate email, DB-write-failure cleanup, P2002 mapping, cleanup-failure-still-propagates, non-fatal email-step failure including non-Error rejection values — and `resendInvite` — happy path, rank denial, unknown-id 404), `apps/api/src/modules/users/__tests__/users-management.controller.spec.ts` (resend-invite forwarding + 404 propagation), `apps/web/src/pages/settings/__tests__/Users.test.tsx` (rank-filtered create-form options, hidden/visible activate-deactivate control, role-change control visibility/options/PATCH/error, resend button visibility/click/error).
