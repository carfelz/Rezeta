...
describe('ProtocolRecommendationsService', () => {
  let repo: { getRecommendations: ReturnType<typeof vi.fn> }
  let svc: ProtocolRecommendationsService

  beforeEach(() => {
    repo = { getRecommendations: vi.fn().mockResolvedValue(sample) }
  ...
  it('clamps limit between 1 and 20 at the controller level (passes limit through)', async () => {
    ...
    expect(repo.getRecommendations).toHaveBeenLastCalledWith('t1', 'd1', 'pat1', 5)
  })
})
