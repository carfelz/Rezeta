...
describe('ProtocolsService — saveVersion publish flow', () => {
  ...
  // ── template schema validation disabled (schema reset v2) ─────────────────
  // templateSchema is hardcoded to { blocks: [] } in the service, so required
  // block validation never fires regardless of protocol content.

  describe('saveVersion — template schema validation disabled post schema-reset-v2', () => {
    it('succeeds even when content section is missing a required placeholder child', async () => {
    ...
    it('succeeds when section and all required child blocks are present in content', async () => {
